...
def create_schema_instance(struct: Any, prop: Tuple[str, Any]) -> Any:
  ...
  try:
    ...
    if field_type in ('list', 'text', 'data'):
      ...
      print(f"Initialized list/text/data field: {field_name}")
    elif field_type in ('struct', 'object'):
      struct_instance.init(field_name)
      print(f"Initialized struct/object field: {field_name}")
...
def compare_schemas(original_instances: List[Any], read_instances: List[Any]) -> bool:
  ...
  compatible = True
  for orig, read in zip(original_instances, read_instances):
    try:
      # Compare the which() method to detect structural changes
      if orig.which() != read.which():
        print(f"❌ Structural change detected: {orig.which()} vs {read.which()}")
        compatible = False
    except Exception as e:
      print(f"Error comparing instances: {e}")
      compatible = False
